  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={openCreate} className="btn-premium text-sm gap-2"><Plus className="w-4 h-4" /> Ajouter</Button>
      </div>
          ...
          <div className="space-y-4 mt-4">
            {fields.map((field) => (
              <div key={field.key}>
                ...
                {field.type === 'textarea' ? (
                  <Textarea value={formData[field.key] || ''} onChange={(e) => setFormData({ ...formData, [field.key]: e.target.value })} className="rounded-xl border-[#E8A0B4]/40" />
                ) : field.type === 'select' ? (
