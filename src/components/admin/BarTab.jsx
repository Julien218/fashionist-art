...
import { QrCode, Loader2, Copy, ExternalLink, AlertTriangle } from 'lucide-react';
...
export default function BarTab({ user }) {
  ...
  const [lastAmountCents, setLastAmountCents] = useState(null);
  ...
  const generateMutation = useMutation({
    mutationFn: async (amountCents) => {
      ...
      console.log('stripeCreateBarCheckout response:', res.data);
    ...
    onSuccess: (data) => {
      if (data?.checkout_url) {
        ...
        setLastAmountCents(data.amount_cents);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Module Bar (affiché seulement si prêt) */}
      {isReady && (
        <>
          {/* Quick amounts */}
          ...
          {qrUrl && checkoutUrl && (
            <div className="bg-white/5 border border-[#FF2D8A]/30 rounded-xl p-6 text-center space-y-4">
              ...
              <p className="text-sm text-white/60">
                ...
                {note && <> &mdash; <span className="text-white/50">{note}</span></>}
              </p>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-white/10">
                      ...
                      <th className="text-left py-2 px-2 text-xs text-white/40 font-display uppercase">Frais</th>
                  ...
                  <tbody>
                    {salesToday.map((s) => (
                      <tr key={s.id} className="border-b border-white/5">
                        ...
                        <td className="py-2 px-2 text-red-400 text-xs">{((s.stripe_fee_cents || 0) / 100).toFixed(2)} €</td>
